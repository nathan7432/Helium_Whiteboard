...
def score():
    ...
    # starting hex_dict that will contain all relevant hexes
    ...
    # position in list for each hex resolution
    hex_addr_key = {4: 8, 5: 7, 6: 6, 7: 5, 8: 4, 9: 3, 10: 2, 11: 1, 12: 0}

    def add_interactive(hspot_by_addr, last_cg, interacitve_var):
        # adds interactive field to hotspot
        interactive_counter_yes = 0  # debug
        interactive_counter_no = 0  # debug
        for hspot in hspot_by_addr:
            if type(hspot_by_addr[hspot]["last_poc_challenge"]) == type(None):
                hspot_by_addr[hspot]["interactive"] = "no"
                interactive_counter_no += 1
            elif hspot_by_addr[hspot]["last_poc_challenge"] > last_cg - interacitve_var:
                hspot_by_addr[hspot]["interactive"] = "yes"
                interactive_counter_yes += 1
            else:
                hspot_by_addr[hspot]["interactive"] = "no"
                interactive_counter_no += 1
        return hspot_by_addr

    hspot_dict = add_interactive(h.hspot_by_addr, h.last_cg, h.interacitve_var)

    def add_parents(hspot_dict):
        # add field to hspot in hspot_by_addr and fill with its hex addresses 12 to 4
        for hspot in hspot_dict:
            try:
                hex12_adr = h3.geo_to_h3([hspot]["lat"], hspot_dict[hspot]["lng"], 12)
                hspot_dict[hspot]["hex_addr"] = hex_parents(hex12_adr)
            except KeyError:
                continue  # no long/lat
        return hspot_dict

    hspot_dict = add_parents(hspot_dict)

    def add_hex_10(hspot_dict, hex_dict):
        # add hex and field n to hex_dict, number of interactive hotspots in hex
        hex_addr_key = {4: 8, 5: 7, 6: 6, 7: 5, 8: 4, 9: 3, 10: 2, 11: 1, 12: 0}
        spot_in_list = hex_addr_key[10]
        for hspot in hspot_dict:
            if hspot_dict[hspot]["interactive"] == "yes":
                hex = hspot_dict[hspot]["hex_addr"][spot_in_list]
                try:
    ...
    hex_dict = add_hex_10(hspot_dict, hex_dict)

    def add_sum_neighbors_10(hex_dict):
    ...
    hex_dict = add_sum_neighbors_10(hex_dict)

    def add_density_lmt_10(hex_dict):
    ...
    hex_dict = add_density_lmt_10(hex_dict)

    def add_scaling_10(hex_dict):
        # add clipped and unclipped scaling to hex res 10
        for hex in hex_dict[10]:
            hex_dict[10][hex]["clipped"] = min(hex_dict[10][hex]["dens_lmt"], hex_dict[10][hex]["sum_clipped_children"])
            hex_dict[10][hex]["unclipped"] = min(hex_dict[10][hex]["dens_lmt"] / hex_dict[10][hex]["sum_clipped_children"],
                                                 1)
        return hex_dict

    hex_dict = add_scaling_10(hex_dict)
    ...
    for res in range(9, 3, -1):
        ...
        for hex in hex_dict[res]:
            hex_dict[res][hex]["clipped"] = min(hex_dict[res][hex]["dens_lmt"],
                                                hex_dict[res][hex]["sum_clipped_children"])
            hex_dict[res][hex]["unclipped"] = min(
                hex_dict[res][hex]["dens_lmt"] / hex_dict[res][hex]["sum_clipped_children"],
                1)
